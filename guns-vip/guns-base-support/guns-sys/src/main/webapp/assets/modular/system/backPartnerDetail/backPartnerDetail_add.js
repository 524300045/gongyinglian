...
layui.use(['form', 'admin','table', 'ax','laydate','upload','formSelects'], function () {
    ...
    DetailTable.initColumn = function () {
        return [[
            ...
            {field: 'goodsName', sort: true, title: '商品名称'},
            {field: 'goodsModel', sort: true, title: '规格'},
            {field: 'planNum',title: '退供数量', minWidth:100, align:"left",edit: 'text',style: 'outline: 1px solid #e6e6e6;outline-offset: -5px;',event:"dataCol"},
            {field: 'unitName', sort: true, title: '单位'},
    ...
    GoodsTable.search = function () {
        ...
        table.reload(GoodsTable.tableId, {
            ...
            url: Feng.ctxPath + '/backPartnerDetail/partnergoodslist'
        });
    };
    ...
    form.on('submit(btnSubmit)', function (data) {
        ...
        {
            if (tableDatas[i].planNum=="")
            {
                msg=msg+tableDatas[i].skuCode+"未填写退供数量,<br/>";
            }
        }
        if (msg!="")
        {
        ...
        var ajax = new $ax(Feng.ctxPath + "/backPartnerDetail/addItemDetail", function (data) {
            if (data.code==200)
            {
                Feng.success("添加成功！");
                rowData=[];
